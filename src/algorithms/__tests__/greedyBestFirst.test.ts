/**
 * Unit tests for Greedy Best-First Search.
 *
 * Tests cover all 4 shared fixture grids:
 *   1. Open field — finds a path (may or may not be optimal).
 *   2. Wall detour — navigates around obstacle.
 *   3. U-trap — solves despite the concave wall (has backtracking, unlike HC).
 *   4. Fully blocked — reports 'failed' when goal is unreachable.
 *
 * Greedy Best-First is NOT optimal, so tests verify path validity
 * (contiguous, avoids walls, reaches the goal) rather than optimal length.
 *
 * @module algorithms/__tests__/greedyBestFirst.test
 */

import { describe, it, expect } from 'vitest';
import { greedyBestFirstSearch } from '../greedyBestFirst';
...
function run(grid: Parameters<typeof greedyBestFirstSearch>[0]) {
  const gen = greedyBestFirstSearch(grid);
...
describe('Greedy Best-First Search', () => {
  ...
  describe('edge cases', () => {
    it('should handle start === goal', () => {
      const gen = greedyBestFirstSearch({
        width: 3,
        height: 3,
