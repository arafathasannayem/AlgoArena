 ...
 *   1. Open field — finds optimal shortest path (both sides BFS ⇒ optimal).
 ...
 * point behavior.
 *
 * @module algorithms/__tests__/bidirectionalBfs.test
 */
...
function run(grid: Parameters<typeof bidirectionalBfs>[0]) {
  const gen = bidirectionalBfs(grid);
...
describe('Bidirectional BFS', () => {
  ...
  describe('generator protocol', () => {
    ...
    it('should yield consider events', () => {
      const { events } = run(OPEN_FIELD);
