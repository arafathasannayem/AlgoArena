 ...
 *
 * Implements Section 5.3 of the UI/UX Guidelines with Light Brick Theme:
 * - 64px wide vertical stack on the left screen rail in Brick White (#F4F4F4).
 ...
 * - Rough Terrain cost selector flyout with presets (×2, ×3, ×5, ×10, ×25) and badge.
...
const COST_PRESETS = [2, 3, 5, 10, 25] as const;
...
export function ToolDock() {
  ...
  return (
    <aside className="fixed left-3 top-20 z-20 flex flex-col items-center select-none">
      {/* 64px wide vertical dock container in Brick White */}
      <div className="w-16 bg-[#F4F4F4] border-[3px] border-[#05131D] rounded-2xl p-2 flex flex-col items-center gap-2.5 shadow-[0_6px_0_rgba(5,19,29,0.35)]">
        {/* Top Decorative 2 Studs */}
        <div className="flex items-center justify-center gap-2 pb-1 border-b-2 border-[#05131D]/15 w-full">
          ...
          return (
            <div key={tool.id} className="relative group">
              <button
                ...
                disabled={isRunning}
                className={`w-11 h-11 rounded-xl flex items-center justify-center brick-btn transition-all relative ${
                  isDestructive
                    ? 'bg-[#582A12] text-[#F4F4F4] hover:bg-[#6e3618]'
                    : isActive
                      ? 'bg-[#C91A09] text-[#F4F4F4] ring-2 ring-[#F2CD37] shadow-[0_2px_0_#05131D]'
                      : 'bg-white hover:bg-[#E8E8E8] text-[#05131D]'
                }`}
                aria-label={tool.label}
              >
                {tool.icon}

                {/* Multiplier badge on Rough Terrain tool */}
                {isCostTool && (
                  <span className="absolute -bottom-1 -right-1 text-[9px] font-mono font-black px-1 rounded bg-[#F2CD37] border border-[#05131D] text-[#05131D] leading-tight shadow-sm">
                    ×{highCostValue}
                  </span>
                )}
              </button>

              {/* Interactive Cost Selector Flyout when Rough Terrain is active */}
              {isCostTool && isActive && !isRunning && (
                <div className="absolute left-14 top-1/2 -translate-y-1/2 z-50 flex items-center gap-1.5 p-1.5 rounded-2xl bg-[#F4F4F4] border-[3px] border-[#05131D] shadow-[0_6px_0_rgba(5,19,29,0.35)] animate-in fade-in-50 zoom-in-95 duration-100 whitespace-nowrap">
                  <span className="text-[10px] font-mono font-black uppercase text-[#595D60] px-1">
                    Cost:
                  </span>
                  {COST_PRESETS.map((val) => (
                    <button
                      ...
                      className={`px-2 py-1 rounded-xl text-xs font-mono font-black border-2 border-[#05131D] transition-all cursor-pointer ${
                        highCostValue === val
                          ? 'bg-[#F2CD37] text-[#05131D] shadow-[0_2px_0_#05131D] -translate-y-0.5 ring-1 ring-[#05131D]'
                          : 'bg-white hover:bg-[#E8E8E8] text-[#05131D]'
                      }`}
                      title={`Set rough terrain cost to ×${val}`}
                    >
                      ×{val}
                    </button>
                  ))}
              ...
              {/* Tooltip on right (hidden when cost flyout is active) */}
              {hoveredTool?.id === tool.id && (!isCostTool || !isActive) && (
                <div className="absolute left-14 top-1/2 -translate-y-1/2 z-50 pointer-events-none pl-2 animate-in fade-in-50 zoom-in-95 duration-100">
                  <div className="bg-[#F4F4F4] text-[#05131D] border-2 border-[#05131D] px-2.5 py-1.5 rounded-xl shadow-[0_4px_0_rgba(5,19,29,0.3)] whitespace-nowrap flex flex-col gap-0.5">
                    ...
                    <span className="text-[10px] text-[#595D60] font-medium font-sans">
                      {isCostTool ? `Paint rough terrain (×${highCostValue})` : tool.desc}
                    </span>
                  </div>
                </div>
              )}
